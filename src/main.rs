// source: https://github.com/bevyengine/bevy/blob/v0.12.1/examples/scene/scene.rs
...
fn log_system(
    ...
    res_a: Option<Res<ResourceA>>,
    res_b: Option<Res<ResourceB>>,
) {
    ...
    if let Some(res) = res_a {
        if res.is_added() {
            info!("  New ResourceA: {{ score: {} }}\n", res.score);
        }
    }
    if let Some(res) = res_b {
        if res.is_added() {
            info!("  New ResourceB: {{ score: {} }}\n", res.score);
        }
    }
}
